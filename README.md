...
2. 找出model对应的属性列表
	...
	1) 通过__runtime__的`class_copyPropertyList`方法获取model除`JSONModel`父类的所有的property列表

	2) 通过`property_getName`获取到属性名和`property_getAttributes`获取到的__`encode string`__，来分析出每个属性各的类型，名称等，并保存到JSONModel的`JSONModelClassProperty`对象里。
...
__keyMapper__为用户自定义的映射表，如果不为空，则根据用户定义的进行转换
